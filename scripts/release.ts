...
const release = (bumpType: BumpType, dryRun: boolean) => {
  ...
  runVisible(
    `gh pr create --title "release: v${newVersion}" --body "Bumps version from ${oldVersion} to ${newVersion}. Merging will auto-publish to npm and create a GitHub release."`,
  );

  console.log(`\nRelease PR created for v${newVersion}.\n`);
  console.log("  Merge the PR on GitHub to trigger the publish workflow.\n");
};
